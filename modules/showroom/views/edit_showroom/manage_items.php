    ...
    foreach($items as $item)
    {
      ...
      <li id="showroom_<?php echo $item->id?>" <?php echo $class?>>
        <table><tr>
          ...
          <td class="page_edit"><a href="/get/edit_showroom/edit?item_id=<?php echo $item->id?>" rel="facebox" class="secondary" id="<?php echo $item->id?>"><?php echo $item->name?></a></td>
          <td width="60px" class="alignright"><a href="/get/edit_showroom/delete?pid=<?php echo "$this->pid&node_id=$item->id"?>" class="delete_showroom_item" id="<?php echo $item->id?>">Delete</a></td>
        </tr></table>
      </li>    
      <?php
      ++$counter;
