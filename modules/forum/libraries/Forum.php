...
class Account_Core {
	...
	/**
	 * Return a static instance of Account.
	 *
	 * @return  object
	 */
	public static function instance($config = array())
	{
		...
		// Load the Account instance
		empty($instance) and $instance = new Account($config);
	...
	public function __construct($config = array())
	{
		...
		$driver = 'Account_'.$config['driver'].'_Driver';
		...
		if ( ! ($driver instanceof Account_Driver))
			throw new Kohana_Exception('core.driver_implements', $config['driver'], get_class($this), 'Account_Driver');

		// Load the driver for access
		$this->driver = $driver;

		Kohana::log('debug', 'Account Library loaded');
	}
	 ...
	 * @return  boolean
	 */
	public function login($username, $fk_site, $password, $remember = FALSE)
	{
		if (empty($password))
			return FALSE;
		...
		return $this->driver->login($username, $fk_site, $password, $remember);
	}
