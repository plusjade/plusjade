...
class Navigation_Core {
	...
	{	  
		...
		echo '<button id="link_save_sort">Save the sort</button>';
		...
		foreach ($items as $item) 
		{
			...
			if($new == $old)
			{
				...
				{
					echo "</li>\n";
					echo '  <li rel="',$item->display_name,'" id="item_' . $item->id . '" class="position_'.++$position_array[$list_id].'" >' . $item->display_name;
				}
			}			
			else
			{
				...
				{		
					for($x = $old ; $x < $new; $x++)
					{
						...
						echo '  <li rel="',$item->display_name,'" id="item_' . $item->id . '" class="position_1" >' . $item->display_name;
					}
