...
class Calendar_Core {
	...
	function day_function($year, $month, $day, $date_data)
	{
		...
		if( 'NULL' != $date_data )
		{
			echo '<a href="/calendar/day/'."$year/$month/$day".'" rel="ajax" class="day_link_simple">'. $day .'</a>';
		}
		else
	...
	function getPhpAjaxCalendarCore($month, $year, $date_array=NULL, $day_function=NULL)
	{
		...
		<table id="calendar">
		<tr>
			<td><a href="/calendar/month/$prev_year/$prev_month" class="monthnav">&laquo; Last</a></td>
			<td colspan="5" class="month">$month_name $year</td>
			<td><a href="/calendar/month/$next_year/$next_month" class="monthnav">Next &raquo;</a></td>
		</tr>
		<tr class="daynames"> 
...
EOS;
		...
		foreach($weeks AS $week)
		{
			...
			foreach($week as $day)
			{
				...
				$has_events = '';
				if(! empty($day) AND isset($date_array[$day]) )
				...
				if (NULL == $day_function OR NULL == $date_array)
				{
					echo '<div class="day_simple">'.$day.'</div>';
				}
				else
	...
	function getPhpAjaxCalendar($month, $year, $date_array=NULL, $day_function=NULL) 
	{
