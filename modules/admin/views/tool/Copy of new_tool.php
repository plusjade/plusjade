		...
		<div id="tool_<?php echo $tool->id?>" class="each_tool">
			<?php echo form::open("tool/add/$page_id", array('class' => 'custom_ajaxForm') )?>
				<div class="desc">
					...
					<?php
						foreach($tool->system_tool_views as $view):
							if(!isset($checked)) $checked = 'CHECKED';
					?>
						<dl>
							<dt><input type="radio" name="view" value="<?php echo $view->view?>" <?php echo $checked?>> <?php echo $view->view?></dt>
							<dd><?php echo $view->desc?></dd>
						</dl>
					<?php
						$checked = '';
						endforeach;
