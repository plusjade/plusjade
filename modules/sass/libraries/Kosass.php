...
class Kosass_Core
{
	...
	/**
	 * Create an instance of Kosass.
	 *
	 * @return  object
	 */
	public static function factory($style, $debug=FALSE)
	{
		return new Kosass($style, $debug);
	}
	...
	public function structure($contents, $script = NULL)
	{
		// if $contents are empty throw error
		if (empty($contents))
			throw new Exception("Kosass template '$script' can not be found.");
		...
		# return the array with only selectors defined.
		#return $output;
 ...
 */
	...
	private function line_type($line, $debug = FALSE)
	{
		...
		{
			str_replace(array('#','.','&','*'), '', $line, $matches);
			if(0 < $matches)
				$type = 'element';
			else
				$type = 'comment';
    ...
    if(empty($type))
    {
			echo "line $this->lineno: $first $line";die();
    }
		return $type;
	}
...
//- line types -------------------------------------------------------
	...
	{			
		...
		# is this element a child ? (element is indented)
		if(($curr_indent != 0))
		{
			# replace any parent references with the parent (&)
			$this->nested[$curr_indent] = str_replace('&', $this->nested[$curr_indent-2], trim($this->line), $matches);
		
		
			# if there's a parent reference, disclude the parent from the parent_string
			$omit = (0 < $matches) ? 4 : 2;
			# create the parent string to prepend to the element names on this line.
			# can have more then one ancentor so we loop.
			$parent_string = '';
			$i = 0;		
			while($i <= $curr_indent-$omit) 
			{
			...
			{
				# is there an ampersand parent reference? # if so replace it with the direct parent.
