...
# edit a site data file
	function edit_file($file, $dir=NULL)
	{
		if('m' == $dir)
		{
			$folder = "modules/$file/";
		...
		if(file_exists($current_file))
		{
			$this->template->rootJS('$.jGrowl("Page Loaded.");'); #status message
			$contents = file_get_contents($current_file);
			return $contents;
		}
		else
			$this->template->rootJS('$.jGrowl("You cannot edit this file.");'); #status message		
	}

# save edits to a file
	function save_file($file_name, $new_contents, $dir=NULL)
	{
		if($dir == 'm')
		{
			$folder = "modules/$file_name/";
			$file_name = 'css';
		}
		else
			$folder = 'global/';		

		$current_file = DOCROOT."data/$this->site_name/themes/$this->theme/$folder$file_name.php";
